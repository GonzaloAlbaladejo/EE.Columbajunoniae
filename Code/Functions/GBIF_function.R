...
Dowload_gbif<-function(sp_list, # (Character) List of species from which to dowload spatial information
                       ...
                       n_records=5000 # Maximun number of records to retrieve
                       ){
  ...
  if(exists("y")){
    ...
    if(is.null(sp_list)){
     ...
     write.csv(y,paste(exit_route,paste0(sp_list,".csv"),sep="/"))
      rm(y)
    }
  ...
  for(f in 1:length(sp_list)){
    ...
    if(exists("y")){
      write.csv(y,paste(Temp_tax,paste0(sp,".csv"),sep="/"))
      rm(y)
    }
   }
  #stopCluster(cl)
  ...
  # erase the temporal folder
    ...
    write.csv(points_spp,paste(exit_route,paste0(sp_list[1],".csv"),sep="/")) # Export the point information under the original name
